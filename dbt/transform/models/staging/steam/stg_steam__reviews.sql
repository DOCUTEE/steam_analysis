{{
    config(
        ...
        file_format='iceberg',
        partition_by=['review_day', 'extract_date']
    )
}}
...
WITH reviews_cleaned AS (
    SELECT 
        ...
        DATE(TO_TIMESTAMP(CAST(timestamp_created AS BIGINT))) AS review_day,
        DATE('{{ var("extract_date", "now()") }}') AS extract_date

    FROM {{ source('steam', 'reviews') }}
    WHERE 
        DATE(TO_TIMESTAMP(CAST(timestamp_created AS BIGINT))) = DATE('{{ var("review_day") }}')
